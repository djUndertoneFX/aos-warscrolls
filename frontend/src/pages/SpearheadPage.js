...
export default function SpearheadPage({ headerCollapsed }) {
  ...
  // Spearhead column collapsed state
  const [spearheadColCollapsed, setSpearheadColCollapsed] = useState(() => {
    try { return JSON.parse(localStorage.getItem('aos-sp-col-collapsed')) ?? false; } catch { return false; }
  });
  const toggleSpearheadCol = () => setSpearheadColCollapsed(v => {
    const next = !v;
    localStorage.setItem('aos-sp-col-collapsed', JSON.stringify(next));
    return next;
  });
  ...
  const DEFAULT_COL_WIDTHS = {
    rownum: 22, friendly: 24, enemy: 24, expand: 22, spearhead: 140, thumb: 44,
  ...
  const STORAGE_KEY = 'aos-sp-col-widths-v1';
  ...
  // # | F | E | expand | spearhead | name | thumb | faction | alliance | pts | mdl | mv | hp | ctrl | sv | wd | types | kw | ADO-R | ADO-M | ADO/e
  const TOTAL_COLS = 21;
  const WEAPONS_EXPAND_COLS = 17;
    ...
    <div className="table-page">
      ...
      {loading ? (
        ...
        <div className="empty-state">
          No warscrolls found. Try adjusting your filters.
        </div>
      ) : (
        <>
            ...
            <table data-sort={sortBy}>
              ...
              <thead>
                <tr>
                  ...
                  {/* ── Spearhead column (collapsible) ── */}
                  <th
                    style={spearheadColCollapsed
                      ? { width: 18, position: 'relative', padding: 0, overflow: 'hidden', cursor: 'pointer' }
                      : { ...thStyle('spearhead'), cursor: 'pointer' }
                    }
                    className="col-spearhead-hdr"
                    title={spearheadColCollapsed ? 'Expand Spearhead column' : 'Spearhead — click to collapse'}
                    onClick={toggleSpearheadCol}
                  >
                    {spearheadColCollapsed
                      ? <span className="sp-col-toggle">▶</span>
                      : <>Spearhead <span className="sp-col-toggle">◀</span><span className="col-resize-handle" onMouseDown={e => { e.stopPropagation(); startResize(e,'spearhead'); }} /></>
                    }
                  </th>
                    ...
                    <React.Fragment key={row.id}>
                      {factionChanged && sortBy === 'faction' && (
                        <tr className="separator-faction">
                          <td colSpan={TOTAL_COLS}>{row.faction}</td>
                        </tr>
                      )}
                      {typeChanged && (sortBy === 'faction' || faction) && (
                        <tr className="separator-type">
                          <td colSpan={TOTAL_COLS}>{unitTypeLabel(row)}</td>
                        </tr>
                      )}
                      <tr
                      ...
                      >
                        ...
                        {/* ── Spearhead cell ── */}
                        <td
                          className="col-spearhead"
                          style={spearheadColCollapsed ? { width: 18, overflow: 'hidden', padding: 0 } : {}}
